<template>
  <div class="container mx-auto p-4">
    ...
    <!-- Tabla -->
    <TableComponent :loader="movementTypeStore.loading" :columns="columns" :data="filteredMovements"
      id="id_movement_type" :flagRestore="showActive" :currentUserCompany="0" @actionSee="handleSee"
      @actionCreate="handleCreate" @actionUpdate="handleUpdate" @actionDanger="handleRemove"
      @actionRestore="handleRestore"></TableComponent>
    ...
    <!-- Modal Crear/Editar -->
    <Dialog v-model:visible="visibleForm" modal
      :header="isEdit ? 'Editar Tipo de Movimiento' : 'Crear Tipo de Movimiento'" :style="{ width: '30rem' }">
      <div class="flex flex-col gap-3">
        <label class="font-semibold">Nombre del tipo *</label>
        <InputText v-model="formMovementName" :class="{ 'p-invalid': nameError }"
          placeholder="Ingrese el nombre del tipo de movimiento"></InputText>
        <small v-if="nameError" class="p-error">{{ nameError }}</small>

        <label class="font-semibold">Clasificación *</label>
        <Dropdown v-model="formClasificationId" :options="movementTypeStore.clasificationMovements"
          optionLabel="name_clasification_movement" optionValue="id_clasification_movement"
          placeholder="Seleccione una clasificación" :class="{ 'p-invalid': clasificationError }"></Dropdown>
