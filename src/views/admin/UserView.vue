<template>
        ...
        <tbody>
          ...
          >
            ...
            <td class="py-2 px-4 border-b">
              <!-- Deshabilitar editar si es el usuario actual -->
              <button
              ...
              <!-- Deshabilitar eliminar si es el usuario actual -->
              <button
                v-if="showActive && user.id_user !== authStore.user?.id_user"
  ...
  const userForm = ref({
    ...
    password_user: '', // Campo de contraseña
  ...
  const openCreateModal = () => {
    ...
    userForm.value = {
      ...
      mail_user: '',
      phone_user: null,
      password_user: '', // Contraseña obligatoria en creación
      id_rol_Id: null,
      id_position_Id: null,
  ...
  const startEdit = (user: User) => {
    ...
    userForm.value = {
      ...
      password_user: '', // Contraseña opcional en edición
      ...
      id_company_Id: companyStore.companies.find(
        (comp) => comp.name_company === user.name_company
      )?.id_company || null,
    };
  };
  ...
  const saveEdit = async () => {
    try {
      const payload = {
        ...userForm.value,
        password_user: userForm.value.password_user || null, // Enviar null si no hay nueva contraseña
      };
  
      // Depuración: Mostrar la estructura del JSON que se enviará
      console.log('Datos del usuario a guardar:', JSON.stringify(payload, null, 2));
