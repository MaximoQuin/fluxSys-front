<template>
  <div class="container mx-auto p-4">
    ...
    <table class="min-w-full bg-white">
      ...
      <tbody>
        <tr v-for="category in filteredCategories" :key="category.id_category_product" class="hover:bg-gray-50">
          ...
          <td class="py-2 px-4 border-b">
            ...
            <div v-else class="flex items-center">
              ...
              />
              <button
                @click="saveEdit(category.id_category_product)"
          ...
          <td class="py-2 px-4 border-b">
            <button
              v-if="showActive && !isEditing(category.id_category_product)"
              @click="startEdit(category.id_category_product, category.name_category_product)"
            ...
            <button
              v-else-if="!isEditing(category.id_category_product)"
              @click="restoreDeletedCategoryProduct(category.id_category_product)"
...
const filteredCategories = computed(() => {
  return categoryProductStore.categoriesProducts.filter(category =>
    showActive.value ? !category.delete_log_category_product : category.delete_log_category_product
  );
});
...
const saveEdit = async (id: number) => {
  if (editingName.value.trim()) {
    await categoryProductStore.editCategoryProduct(id, editingName.value);
    editingId.value = null;
    editingName.value = '';
    await categoryProductStore.fetchCategoriesProducts(); // Refrescar la lista
  }
};
...
const removeCategoryProduct = async (id: number) => {
  await categoryProductStore.removeCategoryProduct(id);
  await categoryProductStore.fetchCategoriesProducts(); // Refrescar la lista
};

const restoreDeletedCategoryProduct = async (id: number) => {
  await categoryProductStore.restoreDeletedCategoryProduct(id);
  await categoryProductStore.fetchCategoriesProducts(); // Refrescar la lista
};

const createCategoryProduct = async () => {
  if (newCategoryName.value.trim()) {
    await categoryProductStore.addCategoryProduct(newCategoryName.value);
