      ...
      <Column v-for="(column, index) in columns" :key="index" :field="column.field" :header="column.header">
        <template #body="props">
          <div v-if="isEditing(props.data[id])">
            ...
            <Button label="Guardar" icon="pi pi-check" class="p-button-success ml-2" @click="saveEdit(props.data[id])" />
            <Button label="Cancelar" icon="pi pi-times" class="p-button-secondary ml-2" @click="cancelEdit(props.data[id])" />
          </div>
          <div v-else>
      ...
      <Column header="Acciones">
        <template #body="props">
          <div v-if="flagRestore" class="flex gap-3">
            ...
            <!-- Botón Eliminar -->
            <Button v-if="hasPermission('Borrar') && props.data.name_company !== currentUserCompany" label="Eliminar" icon="pi pi-times" class="p-button-danger"
              @click="handleDanger(id, props.data)" />
          </div>
          <div v-else>
            <!-- Botón Recuperar -->
